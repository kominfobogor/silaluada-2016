...
*/
class Adm_pengujian_situ extends CI_Controller
{
	...
	function index()
	{
		$data['title'] = "Import Data Pengujian Situ";
		$data['konten'] = 'admin/csv/situ/index';
		$link = 'Adm_pengujian_situ/index/';
		$limit= 10;
		$uri_segment= 3;
		$offset= $this->uri->segment($uri_segment);
		
		$data['list'] = $this->supermodel->getData('pengujian_situ');
		$jum = $this->supermodel->getData('pengujian_situ');
		
		$data['listview'] = $this->supermodel->getData('pengujian_situ',$field='', $order='pengujian_situ_id', $dasc='DESC');
	...
	function importcsv() {
        $data['pengujian_sungai'] = $this->supermodel->getData('pengujian_situ');
        ...
        if (!$this->upload->do_upload()) {
            ...
            if ($this->csvimport->get_array($file_path)) {
                ...
                foreach ($csv_array as $row) {
                    $insert_data = array(
                        'pengujian_situ_id'=>$row['pengujian_situ_id'],
						'nama_situ'=>$row['nama_situ'],
						'lat'=>$row['lat'],
						'lng'=>$row['lng'],
                    ...
                    $this->supermodel->insert_csv('pengujian_situ',$insert_data);
                }
				$data['error'] = "Sukses";
                redirect("adm_pengujian_situ/index");
